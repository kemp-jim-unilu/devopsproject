...
export function seminarListOneGetRequest(id) {
    const url = `seminarlist/${id}`;
   return {
        type: "SEMINAR_LIST_ONE_GET_REQUEST",
       payload: axios.get(url)
   }
}
...
export function fetchRateLimiterStatus() {
  const url = `/ratelimiterstatus`;

  return {
    type: "FETCH_RATELIMITER_STATUS",
    payload: axios.get(url),
  };
}


export function toggleRateLimiter(enabled){
    const url = `/toggleratelimiter`;

    return {
        type: "CHANGE_RATELIMITER",
        payload: axios.put(url, { enabled })
    }
}